...
$lang = array_merge($lang, array(
	...
	<br />
	あなたの情報は２つの方法で発生します。１つは、 “%1$s” のページを閲覧することによって phpBBソフトウェア が cookie をいくつか作成します。cookie 情報はウェブサーバ上にテキストファイルとして作成され、このファイルは閲覧要求の際にローカルコンピュータのウェブブラウザにダウンロードされます。作成される cookie の１番目と２番目はユーザーID（以下 “user-id” ）と匿名セッションID（以下 “session-id” ）であり、これらID情報は phpBBソフトウェア によって自動的にあなたに割り当てられます。作成される cookie の３番目は “%1$s” 内のトピックを閲覧した時に作成されます。この cookie はトピックの既読情報を保管するのに使用され、サイト閲覧の利便性を向上させます。<br />
	<br />
	“%1$s” には、phpBBソフトウェア 以外のソフトウェア（MODなど）によって動作するページがあるかもしれません。それらの中にはアクセスすることによって cookie を作成するものもあるでしょう。しかしこのドキュメントは phpBBソフトウェア の使用によって発生するあなたの情報の取り扱いについて述べたものであり、他のソフトウェアの使用によって発生するあなたの情報については関知しない事を述べておきます。当サイトへアクセスすることによって発生するあなたの情報のもう１つは、あなたが私達に送信するデータです。	具体的には、ゲストユーザーとして投稿したデータ（以下 “ゲストの投稿記事”）、“%1$s” にユーザー登録する際に送信したデータ（以下 “あなたのアカウント情報”）、登録ユーザーとして投稿したデータ（以下 “あなたの投稿記事” ）です。<br />
	<br />
...
$lang = array_merge($lang, array(
	...
	'NO_WATCHED_SELECTED'		=> '監視中のトピックまたはフォーラムが１つも選択されていません',
	...
	'GROUPS_EXPLAIN'	=> 'もしあなたが複数のグループに参加している場合、（管理人がデフォルトグループの変更を許可していれば）それらグループの中から１つだけデフォルトグループを選択できます。デフォルトグループとは、他のユーザーに対してあなたがどのように見えるか、例えばユーザー名の色、アバター画像、ランクなどを決定するためのグループです。（管理人が許可していれば）あなたは他のグループに参加することもできます。参加するグループによっては、それまでアクセスできなかったフォーラムへアクセスできるようになったり使えなかった機能が使えたりするようになるでしょう。',
