...
$lang = array_merge($lang, array(
	'ACP_PERMISSIONS_EXPLAIN'	=> '
		...
		<p>パーミッションの設定と管理についてのより詳しい説明が <a href="http://www.phpbb.com/support/documentation/3.0/quickstart/quick_permissions.html">Chapter 1.5 of our Quick Start Guide</a> に載ってます。</p>
	',

	'ACL_NEVER'				=> 'Never',
	'ACL_SET'				=> 'パーミッションの設定',
	'ACL_SET_EXPLAIN'		=> 'ユーザーまたはグループにパーミッションを割り当てるには、それぞれのユーザーまたはグループに対してパーミッションセットを割り当てるか、カスタム設定でパーミッションオプションを個別に選択する（ “はい” にする）必要があります。<br /><br /><em>パーミッション設定が重複している状況について分かり易く説明するために、例として ユーザーＸ さんが グループＡ と グループＢ のメンバーである状況を想定してみましょう。<br />例１： あるパーミッションオプションが グループＡ で <samp>はい</samp> 、グループＢ で <samp>いいえ</samp> に設定されている場合、たとえ ユーザーＸ で <samp>いいえ</samp> に設定されていたとしても、ユーザーＸ はそのパーミッションオプションの機能を<b>行使できます</b>。<br />例２： あるパーミッションオプションが グループＡ で <samp>はい</samp>、グループＢ で <samp>Never</samp> に設定されている場合、たとえ ユーザーＸ で <samp>はい</samp> に設定されていたとしても、ユーザーＸ はそのパーミッションオプションの機能を<b>行使できません</b>。<br />つまり要約するとパーミッション設定の力関係は <samp>いいえ</samp> ＜ <samp>はい</samp> ＜ <samp>Never</samp> であるという事です。</em><br /><br />通常は <samp>Never</samp> の使用はグループではなく個々のユーザーのみに控えた方が良いでしょう。複数のユーザーまたはグループを同時にパーミッション設定する場合、ユーザー名またはグループ名の隣にあるチェックボックスをチェックする事でパーミッション設定をコピーできます。',
	'ACL_SETTING'			=> '設定値',
	...
	'ACP_GROUPS_PERMISSIONS_EXPLAIN'			=> 'ここではグループに対してグローバルパーミッション（ユーザーパーミッション、グローバルモデレータパーミッション、Adminパーミッション）を割り当てる事ができます。ユーザーパーミッションはアバターやプライベートメッセージの使用などの権限を、グローバルモデレータパーミッションは投稿記事の承認やトピック管理やアクセス禁止などの権限を、Adminパーミッション はパーミッション設定や BBCode のカスタム設定やフォーラム管理などの権限をそれぞれ含みます。ユーザー個々のパーミッションの設定は特別な事情がある場合のみに限るべきです。推奨される方法はユーザーをまずグループに振り分け、そのグループに対してパーミッションを割り当てる方法です。',
	'ACP_ADMIN_ROLES_EXPLAIN'					=> 'ここでは Adminパーミッションセット を管理できます。パーミッションセットとはパーミッションオプションの集まりの事であり、もしパーミッションセットの構成を変更するとそのパーミッションセットを与えられたユーザーとグループのパーミッションも変更されます。',
	'ACP_FORUM_ROLES_EXPLAIN'					=> 'ここではフォーラムパーミッションのパーミッションセットを管理できます。パーミッションセットとはパーミッションオプションの集まりの事であり、もしパーミッションセットの構成を変更するとそのパーミッションセットを与えられたユーザーとグループのパーミッションも変更されます。',
	'ACP_MOD_ROLES_EXPLAIN'						=> 'ここではモデレータパーミッションのパーミッションセットを管理できます。パーミッションセットとはパーミッションオプションの集まりの事であり、もしパーミッションセットの構成を変更するとそのパーミッションセットを与えられたユーザーとグループのパーミッションも変更されます。',
	'ACP_USER_ROLES_EXPLAIN'					=> 'ここではユーザーパーミッションのパーミッションセットを管理できます。パーミッションセットとはパーミッションオプションの集まりの事であり、もしパーミッションセットの構成を変更するとそのパーミッションセットを与えられたユーザーとグループのパーミッションも変更されます。',
	'ACP_USERS_FORUM_PERMISSIONS_EXPLAIN'		=> 'ここではユーザーに対してローカルパーミッション（フォーラムパーミッション、モデレータパーミッション）を割り当てる事ができます。',
	'ACP_USERS_PERMISSIONS_EXPLAIN'				=> 'ここではユーザーに対してグローバルパーミッション（ユーザーパーミッション、グローバルモデレータパーミッション、Adminパーミッション）を割り当てる事ができます。ユーザーパーミッションはアバターやプライベートメッセージの使用などの権限を、グローバルモデレータパーミッションは投稿記事の承認やトピック管理やアクセス禁止などの権限を、Adminパーミッション はパーミッション設定や BBCode のカスタム設定やフォーラム管理の権限などをそれぞれ含みます。多数のユーザーに対して同じ設定をする場合は グローバルパーミッション [ グループ ] を利用した方が良いでしょう。ユーザー個々のパーミッションの設定は特別な事情がある場合のみに限るべきです。推奨される方法はユーザーをまずグループに振り分け、そのグループに対してパーミッションを割り当てる方法です。',
	...
	'NO_ROLE_ASSIGNED_EXPLAIN'	=> 'これを選択してもカスタム設定は変更されません。全てのパーミッションを解除するには “全て<samp>いいえ</samp>” リンクをクリックしてください。',
	...
	'ROLE_DESCRIPTION_FORUM_NOACCESS'		=> 'フォーラムを閲覧する事もアクセスする事もできません',
	...
	'ROLE_DESCRIPTION_FORUM_READONLY'		=> 'フォーラムを閲覧できますが、投稿はできません',
	...
	'ROLE_DESCRIPTION_MOD_SIMPLE'			=> '記事の基本的なアクションのみ実行できます。警告の送信と投稿記事の承認はできません',
	...
	'TRACE_DEFAULT'					=> 'パーミッションのデフォルトは <samp>いいえ</samp>（未設定） です。以下に続く各グループまたはユーザーのパーミッション設定次第で暫定値は <samp>はい</samp> か <samp>NEVER</samp> に上書きされます。',
	'TRACE_FOR'						=> 'トレース',
	...
	'TRACE_USER_FOUNDER'					=> 'このユーザーはウェブマスターなので、Adminパーミッション はデフォルトで <samp>はい</samp> に設定されています',
