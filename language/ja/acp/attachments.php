...
$lang = array_merge($lang, array(
	...
	'ACP_ORPHAN_ATTACHMENTS_EXPLAIN'	=> 'ここでは孤立ファイルを管理できます。ユーザーが投稿画面でファイルを記事に添付したにもかかわらず、肝心の記事を投稿しなかった場合、その添付ファイルは孤立します。ここではそれら孤立したファイルを削除または再添付することが可能です。投稿済みの記事に孤立したファイルを再添付するには添付先記事の ID ナンバーが必要であり、この ID ナンバーはご自分で調べる必要があります。添付先記事の ID を孤立したファイルに割り当てればそのファイルはその記事において表示されます。',
	...
	'ATTACH_QUOTA_EXPLAIN'				=> '掲示板が保管しておける全添付ファイルの限界容量です。0 に設定すると上限はありません。',
	'ATTACH_TO_POST'					=> '再添付',
	...
	'CREATE_THUMBNAIL_EXPLAIN'	=> '画像ファイルをサムネイル表示します',

	'DEFINE_ALLOWED_IPS'			=> 'IPアドレス/ホスト名 の認可',
	'DEFINE_DISALLOWED_IPS'			=> 'IPアドレス/ホスト名 の否認',
	'DOWNLOAD_ADD_IPS_EXPLAIN'		=> '１行につき１つの IPアドレス/ホスト名 を入力してください。IPアドレスを範囲指定する場合、数字と数字をハイフン(-)でつないでください。部分一致としてワイルドカード(*)を使用できます。',
	'DOWNLOAD_REMOVE_IPS_EXPLAIN'	=> '複数選択するにはマウスとキーボードをうまく活用してください（ Shiftキー または Ctrlキー を押しながらクリック）。',
	...
	'MAX_EXTGROUP_FILESIZE'			=> '最大ファイルサイズ',
	...
	'REMOVE_ALLOWED_IPS'		=> '<em>ダウンロードリスト [ 認可 ]</em> から IPアドレス/ホスト名 を削除',
	'REMOVE_DISALLOWED_IPS'		=> '<em>ダウンロードリスト [ 否認 ]</em> から IPアドレス/ホスト名 を削除',

	'SEARCH_IMAGICK'				=> 'ImageMagick を探す',
	'SECURE_ALLOW_DENY'				=> 'ダウンロードリストのタイプ',
	'SECURE_ALLOW_DENY_EXPLAIN'		=> 'セキュアダウンロードが有効な場合、認可/否認 のどちらをリストとして使用するかを選択します',
