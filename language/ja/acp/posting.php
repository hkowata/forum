...
$lang = array_merge($lang, array(
	...
	'HTML_REPLACEMENT_EXPLAIN'		=> 'BBCode を含む文章をブラウザで表示する際、文章内の各 BBCode 部分をそれぞれ対応する HTMLコード に置き換える必要があります。ここでは上で定義した BBCode に対応する HTML表現 を定義できます。上で使用したトークン { } を必ず用いるようにしてください。',
...
$lang = array_merge($lang, array(
	'ACP_ICONS_EXPLAIN'		=> 'ここではトピックアイコンの追加、削除、編集を行う事ができます。これらのアイコンはフォーラム閲覧ページにおいてトピックまたは返信記事の件名と並べて表示されます。アイコンパックのインストールと作成を行う事もできます。',
...
// Disallow Usernames
$lang = array_merge($lang, array(
	'ACP_DISALLOW_EXPLAIN'	=> 'ここでは否認ユーザー名の追加と解除を行う事ができます。否認ユーザー名をユーザー登録の際に使用する事はできません。部分一致としてワイルドカード(*)を使用できます。すでに存在しているユーザー名を否認ユーザー名リストに加える事はできない点に注意してください。',
	'ADD_DISALLOW_EXPLAIN'	=> '部分一致としてワイルドカード(*)を使用できます。',
	'ADD_DISALLOW_TITLE'	=> '否認ユーザー名の追加',

	'DELETE_DISALLOW_EXPLAIN'	=> 'ユーザー名を否認ユーザー名リストから解除する事ができます。',
	'DELETE_DISALLOW_TITLE'		=> '否認ユーザー名の解除',
	'DISALLOWED_ALREADY'		=> '入力したユーザー名は既に否認ユーザー名リストに存在しているか、言語フィルターリストに存在しているか、ユーザー登録されています。',
	'DISALLOWED_DELETED'		=> '否認ユーザー名の解除に成功しました',
	'DISALLOW_SUCCESSFUL'		=> '否認ユーザー名の追加に成功しました',

	'NO_DISALLOWED'				=> '否認されたユーザー名はありません',
	'NO_USERNAME_SPECIFIED'		=> 'ユーザー名を指定していません',
));

// Reasons
$lang = array_merge($lang, array(
	'ACP_REASONS_EXPLAIN'	=> 'ここでは理由を管理できます。理由は ユーザーが投稿記事をモデレータにリポートする時 と モデレータが記事の承認を却下する時 の選択肢として使用されます。デフォルト理由（ * でマークされています）は削除できません。デフォルト理由は該当する理由がない場合に使用されます。',
	'ADD_NEW_REASON'		=> '理由の追加',
	'AVAILABLE_TITLES'		=> 'ローカライズ済みの理由タイトル',
	
	'IS_NOT_TRANSLATED'			=> 'ローカライズされて<strong>いません</strong>',
	'IS_NOT_TRANSLATED_EXPLAIN'	=> 'この理由はローカライズされて<strong>いません</strong>。つまり “掲示板の言語” を “日本語” に設定しているユーザーに対しては、この理由のタイトルと説明が翻訳されずにそのまま表示されます。理由のタイトルと説明が日本語の場合は問題ありませんが、日本語以外の場合はローカライズしたほうが良いでしょう。ローカライズするには言語ファイル（language/ja/mcp.php）に理由を追加してください。具体的には、report_reasons 配列内の各連想配列 TITLE と DESCRIPTION に値を追加する必要があります。',
	'IS_TRANSLATED'				=> 'ローカライズされています',
	'IS_TRANSLATED_EXPLAIN'		=> 'この理由はローカライズされています。つまり理由タイトルが言語ファイル（language/ja/mcp.php）で定義されているため、その定義されたタイトルと説明が使用されます。',
	...
	'REASON_EDIT'				=> 'リポート/却下 の理由の編集',
	'REASON_EDIT_EXPLAIN'		=> 'ここでは理由の追加と編集を行う事ができます。理由がローカライズ済みの場合（言語ファイルで定義されている場合）はそのローカライズされた理由が使用されます。',
