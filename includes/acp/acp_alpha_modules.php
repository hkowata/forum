...
*
* @package acp
* @version $Id: acp_alpha_modules.php,v 1.49 2007/10/20 19:14:42 acydburn Exp $
* @copyright (c) 2007 phpALPHA Team, based on acp_modules.php by phpBB Group 
...
class acp_alpha_modules
{
	...
	function main($id, $mode)
	{
		...
		$user->add_lang('mods/alpha_modules');
		$this->tpl_name = 'acp_alpha_modules';
		...
		switch ($action)
		{
			case 'delete':
				...
				if (request_var('parent_id', '') == 0)
				{
					trigger_error($user->lang['CANNOT_DELETE_CAT'] . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id), E_USER_WARNING);
				...
				if (confirm_box(true))
				{
					...
					$db->sql_freeresult($result);
					if ($row['module_basename'] == 'pages')
					{
						include_once($phpbb_root_path . 'includes/ap/info/ap_' . $row['module_basename'] . '.' . $phpEx);
						$module_info = 'ap_' . $row['module_basename'] . '_info';
						$uninstall_module = new $module_info();
						$uninstall_module->uninstall($row['page']);
						unset($uninstall_module);
					}
					...
					$row = $db->sql_fetchrow($result);
					$db->sql_freeresult($result);
					if ($row['module_basename'] == 'portal')
					{
						include_once($phpbb_root_path . 'includes/ap/info/ap_' . $row['module_basename'] . '.' . $phpEx);
						$module_info = 'ap_' . $row['module_basename'] . '_info';
						$uninstall_module = new $module_info();
						$uninstall_module->uninstall($module_id);
						unset($uninstall_module);
					}
					...
					if (!sizeof($errors))
					{
						$module_delete_lang = ($row['module_basename'] == 'pages') ? $user->lang['PAGE_DELETED'] : $user->lang['MODULE_DELETED'];
						$this->remove_cache_file();
						trigger_error($module_delete_lang . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id));
					}
				}
				else
				{
			...
			case 'disable':
				...
				$sql = 'UPDATE ' . ALPHA_MODULES_TABLE . ' 
			...
			case 'quickadd':
				...
				if (request_var('parent_id', '') == 0)
				{
					trigger_error($user->lang['MUST_SELECT_CAT'] . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id), E_USER_WARNING);
				}
				...
				if (confirm_box(true))
				{
					...
					if (isset($fileinfo['modes'][$module_mode]))
					{
						$module_data = array(
							...
							'page_alias'		=> ($module_basename == 'pages') ? 'test_page' : '',
						);

						if ($module_data['module_basename'] == 'pages')
						{
							include_once($phpbb_root_path . 'includes/ap/info/ap_' . $module_data['module_basename'] .'.' . $phpEx);
							$module_info = 'ap_' . $module_data['module_basename'] . '_info';
							$install_module = new $module_info();
							if ($module_data['parent_id'] == 6)
							{
								$module_data['page_alias'] = $install_module->install($module_data['page_alias'], '', '', 1);
							}
							else
							{
								$module_data['page_alias'] = $install_module->install($module_data['page_alias']);
							}
						...
						if ($module_data['module_basename'] <> 'pages')
						{
							include_once($phpbb_root_path . 'includes/ap/info/ap_' . $module_basename .'.' . $phpEx);
							$module_info = 'ap_' . $module_basename . '_info';
							$install_module = new $module_info();
							$install_module->install($module_data['module_id']);
							unset($install_module);
						}
						...
						if (!sizeof($errors))
						{
							...
							if ($module_data['module_basename'] == 'pages')
							{
								trigger_error($user->lang['PAGE_CREATED'] . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id));
							}
							else
							{
			...
			case 'edit':
				...
				if (request_var('parent_id', '') == 0)
				{
					trigger_error($user->lang['CANNOT_EDIT_CAT'] . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id), E_USER_WARNING);
			...
			case 'add':
				...
				$module_data['module_basename'] = (request_var('module_type', '') == 'page') ? 'pages' : request_var('module_basename', (string) $module_row['module_basename']);
				...
				$module_data['module_mode'] = (request_var('module_type', '') == 'page') ? 'pages' : request_var('module_mode', (string) $module_row['module_mode']);
				$module_data['page_alias'] = ($module_data['module_basename'] == 'pages') ? request_var('page_alias', (string) $module_row['page_alias']) : '';
				if ($action == 'edit')
				{
					if ($module_data['parent_id'] == 0)
					{
						trigger_error($user->lang['CANNOT_EDIT_CAT'] . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id), E_USER_WARNING);
	
					}
					$module_info = 'ap_' . $module_data['module_basename'] . '_info';
					$module_path = $phpbb_root_path . 'includes/ap/info/ap_' . $module_data['module_basename'] .'.' . $phpEx;
					if (!class_exists($module_info))
					{
						include($module_path);
					}
					$module_settings_edit = new $module_info();
					$module_settings_edit->edit($module_data['module_mode']);
					$module_settings_info_function = $module_settings_edit->module();
					$module_settings_mode = $module_data['module_mode'];
					if (isset($module_settings_info_function['modes'][$module_settings_mode]['edit_tpl']))
					{
						$template->set_filenames(array(
							'module_settings_edit'    => $module_settings_info_function['modes'][$module_settings_mode]['edit_tpl'],
						));
						$include_module_settings = $template->assign_display('module_settings_edit', '', true);
						$template->assign_vars(array(
							'S_MODULE_SETTINGS'		=> $include_module_settings
						));
					}
				}
				else
				...
				if ($submit)
				{
					...
					if ($action == 'add')
					{
						...
						if (!class_exists($module_info))
						{
							include($phpbb_root_path . 'includes/ap/info/ap_' . $module_data['module_basename'] .'.' . $phpEx);
						}
						$install_module = new $module_info();
						if ($module_data['module_basename'] == 'pages')
						{
							$alias = request_var('page_alias', '', true);
							$module_data['page_alias'] = preg_replace('/\W/', '', $alias);
							$title = request_var('page_title', '', true);
							$content = request_var('page_content', '', true);
							$in_menu = request_var('in_menu', '', true);
							$welcome = request_var('welcome', '', true);
							$module_data['page_alias'] = $install_module->install($module_data['page_alias'], $title, $content, $in_menu, $welcome);
						}
						unset($install_module);
					}
					...
					if ($action == 'edit')
					{
						...
						if ($module_data['module_basename'] == 'pages')
						{
							$module_data['page_alias'] = $module_settings_edit->edit($module_data['module_mode'], true);
						}
						else
						{
							$module_settings_edit->edit($module_data['module_mode'], true);
						}
						unset($module_settings_edit);
					}
					...
					if ($module_data['module_basename'] <> 'pages' && $action == 'add')
					{
						$module_info = 'ap_' . $module_data['module_basename'] . '_info';
						if (!class_exists($module_info))
						{
					...
					if (!sizeof($errors))
					{
						...
						$module_lang = ($action == 'add') ? $user->lang['MODULE_ADDED'] : $user->lang['MODULE_EDITED'];
						$page_lang = ($action == 'add') ? $user->lang['PAGE_CREATED'] : $user->lang['PAGE_UPDATED'];
						$update_create_lang = ($module_data['module_basename'] == 'pages') ? $page_lang : $module_lang;
						trigger_error($update_create_lang . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id));
					}
				}

				// Category/not category?
				$is_cat = (!$module_data['module_basename']) ? true : false;
				$is_module = ($module_data['module_basename']) ? true : false;
				if ($action == 'edit')
				{
					$is_page = ($module_data['module_basename'] == 'pages') ? true : false;
				}
				else
				{
					$is_page = true;
				...
				$template->assign_vars(array_merge(array(
					'S_EDIT_MODULE'		=> true,
					'S_IS_CAT'			=> $is_cat,
					'S_IS_MODULE'		=> $is_module,
					'S_IS_PAGE'			=> $is_page,
					'S_NEW_MODULE'		=> ($action == 'add') ? true : false,
	...
	*/
	function get_module_branch($module_id, $type = 'all', $order = 'descending', $include_module = true)
	{
		...
		$sql = 'SELECT m2.*
			FROM ' . ALPHA_MODULES_TABLE . ' m1
			LEFT JOIN ' . ALPHA_MODULES_TABLE . " m2 ON ($condition)
	...
	*/
	function remove_cache_file()
	{
		...
		$cache->destroy('sql', ALPHA_MODULES_TABLE);
	}
	...
	*/
	function update_module_data(&$module_data, $run_inline = false)
	{
		...
		if (!isset($module_data['module_id']))
		{
			...
			$sql = 'INSERT INTO ' . ALPHA_MODULES_TABLE . ' ' . $db->sql_build_array('INSERT', $module_data);
			$db->sql_query($sql);
	...
	*/
	function delete_module($module_id)
	{
		...
		// If not move
		$diff = 2;
		$sql = 'DELETE FROM ' . ALPHA_MODULES_TABLE . "
			WHERE module_class = '" . $db->sql_escape($this->module_class) . "'
				AND module_id = $module_id";
		$db->sql_query($sql);
